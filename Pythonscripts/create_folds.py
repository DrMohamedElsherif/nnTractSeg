...
if __name__ == "__main__":
    ...
    parser.add_argument("--nnunet_raw", default="/home/m512f/dev/HCP-nnUnetSetup/nnunet_raw",
                        help="Base nnUNet_raw folder (default: /home/m512f/dev/HCP-nnUnetSetup/nnunet_raw)")
    args = parser.parse_args()

    dataset_dir = find_latest_dataset(args.nnunet_raw)
    mapping_file = os.path.join(dataset_dir, "patient_id_mapping.txt")
    output_file = os.path.join(dataset_dir, "splits_final.json")

    if not os.path.exists(mapping_file):
        print(f"❌ Mapping file not found in {dataset_dir}")
        sys.exit(1)
